...
export class MutationsApi {
     ...
     * @throws {@link UnauthorizedError}
     */
    async createContacts(accessToken: string, userIdList: number[]): Promise<Placeholder> {
        const response = await queryOrMutate(this.protocol, this.apiUrl, {
            query: `
     ...
     * @throws {@link UnauthorizedError}
     */
    async deleteContacts(accessToken: string, userIdList: number[]): Promise<Placeholder> {
        const response = await queryOrMutate(this.protocol, this.apiUrl, {
            query: `
    ...
    /**
     * Records that the user received or read the message. If the status is "read", and there's no "delivered" record,
     * the delivery status will be created. Nothing will happen if the status already exists.
     * @throws {@link InvalidMessageIdError} The message doesn't exist in a chat the user is in, or the message is the user's
     * own.
     * @throws {@link InternalServerError}
     * @throws {@link ConnectionError}
     * @throws {@link UnauthorizedError}
    ...
    /**
     * Only an admin can perform this operation. Messages sent by, and polls voted on, by removed users will remain.
     * Users who aren't in the chat, and nonexistent users will be ignored.
     * @throws {@link InvalidUserIdError} The user attempted to leave the chat but they must first appoint another user as an
     * admin because they're the last admin of an otherwise nonempty chat.
     * @throws {@link InternalServerError}
     * @throws {@link ConnectionError}
     * @throws {@link UnauthorizedError}
     */
    ...
    /**
     * Creates a group chat. The {@link GroupChatInput.userIdList} and {@link GroupChatInput}.adminIdList} needn't
     * contain the user's own ID, as it's implicitly included. Nonexistent users are ignored.
    ...
    /**
     * Sends the text in the chat. For example, a restaurant's bot asks if the user wants a burger or a pizza in the
     * text, and the actions are "burger" and "pizza". If the chat is a broadcast chat, the user must be an admin to
     * message.
     *
     * A frontend UI could display this message like a regular text message but with buttons below it. Action messages
     * are meant for bots; human users shouldn't be able to create them. Only the creator of the action message will be
     ...
     * @throws {@link InvalidChatIdError} The user isn't in the chat.
     * @throws {@link InvalidInvitedChatError} The invited chat ID isn't a group chat, or the chat has invitations turned off.
     * @throws {@link InvalidMessageIdError} The context message doesn't exist.
     * @throws {@link InternalServerError}
     * @throws {@link ConnectionError}
     * @throws {@link UnauthorizedError}
     ...
     * @throws {@link InvalidMessageIdError} The message isn't in a chat the user is in, the message isn't visible to the user
     * because they deleted the private chat, or the message isn't the user's own
     * @throws {@link InternalServerError}
     * @throws {@link ConnectionError}
     * @throws {@link UnauthorizedError}
     */
