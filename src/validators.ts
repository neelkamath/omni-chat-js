import {AccountInput, AccountUpdate, ActionMessageInput, GroupChatInput, Login, PollInput} from './graphql-api/models';
...
/** @throws {@link UsernameScalarError} if the value is neither `null` nor a valid {@link Username}. */
...
/** @throws {@link NameScalarError} if the value is neither `null` nor a valid {@link Name}. */
...
/** @throws {@link PasswordScalarError} if the value is neither `null` nor a valid {@link Password}. */
export function validatePasswordScalar(value: string | null): void {
    if (value !== null && value.match(/\s/) !== null) throw new PasswordScalarError();
}
 ...
 */
export function validateAccountInput({username, password, firstName, lastName, bio}: AccountInput): void {
 ...
 * @throws {@link BioScalarError}
 */
export function validateAccountUpdate({username, password, firstName, lastName, bio}: AccountUpdate): void {
    validateUsernameScalar(username);
    validatePasswordScalar(password);
    validateNameScalar(firstName);
    validateNameScalar(lastName);
 ...
 */
export function validateGroupChatInput({title, description}: GroupChatInput): void {
    validateGroupChatTitleScalar(title);
    validateGroupChatDescriptionScalar(description);
}

/** @throws {@link MessageTextScalarError} */
export function validateActionMessageInput({text, actions}: ActionMessageInput): void {
