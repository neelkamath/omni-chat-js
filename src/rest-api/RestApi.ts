...
import {MessageTextScalarError} from '../validators';
...
export class RestApi {
    ...
    async getProfilePic(userId: number, picType: PicType): Promise<Pic | null> {
        const params = new URLSearchParams({'user-id': userId.toString(), 'pic-type': picType}).toString();
    ...
    /**
     * Retrieves the group chat's pic. An access token needn't be sent if the chat is public. Otherwise, the user must
     * be a participant to view the pic.
    ...
    async getGroupChatPic(
        ...
        const params = new URLSearchParams({'chat-id': chatId.toString(), 'pic-type': picType}).toString();
    ...
    async patchGroupChatPic(accessToken: string, chatId: number, pic: File): Promise<void> {
        const params = new URLSearchParams({'chat-id': chatId.toString()}).toString();
        const formData = new FormData();
        formData.append('pic', pic);
    ...
    /**
     * Reads the pic from a message. To get the caption, use the GraphQL API. You needn't pass an access token if the
     * chat is public.
     * @throws {@link UnauthorizedError}
     * @throws {@link ConnectionError}
     * @throws {@link InternalServerError}
